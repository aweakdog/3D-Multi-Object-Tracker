from dataset.semantic_kitti_dataset import SemanticKittiTrackingDataset
...
def track_one_seq(seq_id,config):
    ...
    tracker = Tracker3D(box_type="SemanticKitti", tracking_features=False, config = config)
    dataset = SemanticKittiTrackingDataset(dataset_path, seq_id=seq_id, ob_path=detections_path,type=[tracking_type])
